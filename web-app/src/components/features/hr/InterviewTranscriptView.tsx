...
const TurnCardItem: React.FC<{
  ...
  sessionId?: string;
  defaultOpen?: boolean;
}> = ({ q, idx, sessionId, defaultOpen = false }) => {
  ...
  const displayScore = (() => {
    ...
    return q.score > 0 && q.score <= 10 ? Math.round(q.score * 10) : q.score;
  })();
      ...
      >
        <CardHeader className="bg-slate-50/70 border-b border-slate-100 py-3 px-5 flex flex-wrap items-center justify-between gap-2 min-w-0">
          <div className="flex items-center space-x-2 flex-wrap gap-1 min-w-0 flex-1">
            ...
            {q.topicTag && (
              <Badge variant="outline" className="bg-white text-slate-700 border-slate-300 text-xs whitespace-nowrap">
                {q.topicTag}
              </Badge>
            )}
            {q.isDeepDive && (
              <Badge className="bg-purple-100 text-purple-800 border-purple-200 text-xs gap-1 shrink-0">
                <Search className="size-3 text-purple-600" /> Hỏi Sâu
              </Badge>
            )}
            {/* One-line question preview when collapsed */}
            {!isOpen && q.question && (
              <span className="text-xs text-slate-500 truncate max-w-[280px] hidden sm:block">
                {q.question}
              </span>
            )}
            {isSaved && (
              <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 shrink-0">
                <CheckCircle2 className="size-3 text-emerald-600" /> Đã đánh giá
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {displayScore !== null && (
              <div className="flex items-center space-x-1.5 bg-white px-3 py-1 rounded-lg border border-slate-200">
                <span className="text-[11px] font-semibold text-slate-500">Điểm:</span>
                <span
                  className={`text-xs font-bold ${
                    (displayScore as number) >= 80 ? 'text-emerald-600' : (displayScore as number) >= 60 ? 'text-amber-600' : 'text-rose-600'
                  }`}
                >
                  {displayScore}/100
                </span>
        ...
        <CardContent className="p-5 space-y-4 min-w-0">
          ...
          {(q.strengths || q.improvements || q.suggestedAnswer) && (
            <div className="grid gap-3 pt-2 sm:grid-cols-2 min-w-0">
              ...
              {(q.improvements || q.suggestedAnswer) && (
                <div className="rounded-lg border border-amber-200 bg-amber-50/50 p-3 text-xs text-amber-950 space-y-1.5 min-w-0">
                  {q.improvements && (
                    <>
                  ...
                  {/* Suggested answer — compact inline, folded into this card */}
                  {q.suggestedAnswer && (
                    <p className="text-[11px] text-amber-800 border-t border-amber-200 pt-1.5 leading-relaxed break-words">
                      <Lightbulb className="size-3 text-amber-600 inline mr-1 shrink-0" />
                      <span className="font-semibold">Gợi ý chuẩn mẫu:</span>{' '}
                      {q.suggestedAnswer.length > 180
                        ? q.suggestedAnswer.substring(0, 180) + '…'
                        : q.suggestedAnswer}
                    </p>
                  )}
                </div>
...
export const InterviewTranscriptView: React.FC<InterviewTranscriptViewProps> = ({
  ...
  // The first unreviewed turn opens by default
  const firstUnreviewedIdx = questions.findIndex((q) => !q.hrRating && !q.hrFeedback);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-600">
              Tổng số lượt hỏi đáp: <strong>{questions.length} câu hỏi</strong> | Bấm vào từng lượt để mở rộng và chấm điểm.
            </p>
          </div>
        </div>
      </Card>

      {/* Questions & Responses List — accordion */}
      <div className="space-y-3 min-w-0">
        {questions.map((q, idx) => (
          <TurnCardItem
            ...
            idx={idx}
            sessionId={sessionId}
            defaultOpen={idx === (firstUnreviewedIdx >= 0 ? firstUnreviewedIdx : 0)}
          />
        ))}
      </div>
    </div>
